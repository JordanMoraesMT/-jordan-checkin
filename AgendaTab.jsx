...
import { PG, toLocalDate, todayLocal, CATS, CANAIS, CC, USERS, SECTORS, BRANDS, geoEstimate, S, fD, fDS, hav, DASH, gps, roadKm, csv, fixMojibake } from "../lib";
...
function PdvsTab({visible,orgs,allOrgs,setOrgs,visits,plocs,active,ldId,geoErr,user,token,syncing,syncMsg,onSync,onCheckin,onCheckout,onEdit,onPerson,onQuick,onOpenFicha,focusReq,rfv,excl}){
  ...
  const[fRfv,setFRfv]=useState([]);const[fSr,setFSr]=useState([]);const[fInd,setFInd]=useState([]);const[canalSel,setCanalSel]=useState([]);
  ...
  const fo=useMemo(()=>{let list=orgs;
    ...
    if(canalSel.length)list=list.filter(o=>canalSel.includes(o.canal||"Presencial"));
    ...
    return list;
  },[orgs,search,catSel,citySel,ufSel,segSel,ownerSel,grupoSel,canalSel,visitMode,visitFrom,visitTo,visitsByOrg,lastVisits,nearMe,plocs,sortMode,rfvDe,fRfv,fSr,fInd,excl]);
  ...
  return(<div style={{display:visible?"block":"none"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:8,marginBottom:11}}>
          ...
          <MultiSelect values={canalSel} onChange={v=>{setCanalSel(v);setVc(PG);}} placeholder="Canal" allLabel="Todos" options={CANAIS}/>
        ...
        {orgs.length>0&&<><div style={{display:"flex",alignItems:"baseline",justifyContent:"space-between",margin:"0 4px 12px",gap:8,flexWrap:"wrap"}}>
          ...
          <button onClick={()=>{const rows=[["Nome","CNPJ","Endereço","Bairro","Cidade","UF","Categoria","Canal","Segmento","Produtos","Responsável","Grupo","Dt Última Visita","Visitado por","Dias s/ Visita","Classe RFV","Status Recompra","Última Compra","Fat. 12m"]];fo.forEach(o=>{const lv=lastVisits[o.id];const dias=lv?Math.floor((Date.now()-new Date(lv.time))/86400000):"";const rv=rfvDe(o);rows.push([o.name,o.cnpj||"",`${o.addr?.street||""} ${o.addr?.number||""}`.trim(),o.addr?.district||"",o.addr?.city_name||o.addr?.city||"",o.addr?.state||"",o.cat||"",o.canal||"Presencial",o.sector||"",o.products||"",o.owner||"",o.grupo?.replace("Grupo: ","")||"",lv?fD(lv.time):"Sem visita",lv?lv.who:"",dias,rv?rv.rfv:"",rv?rv.status:"",rv&&rv.ultima?fD(rv.ultima+"T12:00"):"",rv?rv.fat12m:""]);});csv(rows,`clientes-filtrados-${fD(new Date())}.csv`);}} style={{width:"100%",marginTop:8,padding:12,fontSize:13,background:S.pri+"22",border:`1px solid ${S.pri}55`,color:S.pl,fontWeight:500}}>📊 Exportar {fo.length} clientes (Excel)</button>
