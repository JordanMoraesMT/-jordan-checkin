...
function NoteModal({org,onSave,onCancel}){const[n,setN]=useState("");return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:50}}><div style={{background:"#fff",borderRadius:"16px 16px 0 0",padding:"1.25rem",width:"100%",maxWidth:480}}><p style={{fontWeight:500,fontSize:15,margin:"0 0 4px"}}>Observações do check-out</p><p style={{fontSize:12,color:"#6B7280",margin:"0 0 12px"}}>{org?.name||org?.nickname}</p><textarea value={n} onChange={(e)=>setN(e.target.value)} placeholder="O que aconteceu? (opcional)" rows={3} style={{width:"100%",boxSizing:"border-box",marginBottom:12,resize:"vertical",padding:"10px",border:"1px solid #D1D5DB",borderRadius:8,fontSize:14}}/><div style={{display:"flex",gap:8}}><button onClick={onCancel} style={{flex:1,padding:"10px",border:"1px solid #D1D5DB",borderRadius:8,background:"#fff",cursor:"pointer"}}>Cancelar</button><button onClick={()=>onSave(n)} style={{flex:1,background:"#DC2626",color:"#fff",border:"none",fontWeight:500,padding:"10px",borderRadius:8,cursor:"pointer"}}>Finalizar</button></div></div></div>);}
  ...
  return(<div style={{paddingBottom:70}}>
    ...
    {tab==="config"&&(<div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:16}}>
        ...
        <button onClick={()=>{setVisits([]);saveJSON("jc:visits",[]);}} style={{width:"100%",fontSize:13,padding:"10px",border:"1px solid #D1D5DB",borderRadius:8,background:"#fff",cursor:"pointer",color:"#D97706"}}>🗑 Limpar visitas</button>
        <button onClick={()=>{setPdvLocs({});saveJSON("jc:pdvLocs",{});}} style={{width:"100%",fontSize:13,padding:"10px",border:"1px solid #D1D5DB",borderRadius:8,background:"#fff",cursor:"pointer",color:"#D97706"}}>📍 Limpar GPS dos PDVs</button>
