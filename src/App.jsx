  ...
  return(<div id={"org-"+org.id} style={{background:isA?S.cl:S.card,border:`${isA?2:1}px solid ${isA?S.pri:S.brd}`,borderRadius:12,padding:"12px 14px"}}>
    <div style={{display:"flex",justifyContent:"space-between",gap:8}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:4,flexShrink:0}}>
        {isA?<button onClick={()=>onOut(org)} disabled={ldId===org.id} style={{background:S.dng,border:"none",color:"#fff",fontSize:12,fontWeight:700,padding:"10px 18px",borderRadius:50,display:"flex",alignItems:"center",gap:6,boxShadow:`0 2px 10px ${S.dng}44`,cursor:"pointer"}}><LogOut size={20} strokeWidth={2.2}/>{ldId===org.id?"...":"Check-out"}</button>
        :<div style={{display:"flex",gap:5}}>
          <button onClick={()=>onIn(org)} disabled={!!active||ldId===org.id} style={{background:active?S.cl:S.acc,border:"none",color:"#fff",fontSize:12,fontWeight:700,padding:"10px 18px",borderRadius:50,opacity:active?0.4:1,display:"flex",alignItems:"center",gap:6,boxShadow:active?"none":`0 2px 10px ${S.acc}44`,cursor:"pointer"}}><LogIn size={20} strokeWidth={2.2}/>{ldId===org.id?"...":"Check-in"}</button>
          <button onClick={()=>onQuick&&onQuick(org,"WHATSAPP")} style={{width:44,height:44,borderRadius:50,background:S.ok+"20",border:`1.5px solid ${S.ok}55`,color:S.ok,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><MessageCircle size={26} strokeWidth={2.2}/></button>
          <button onClick={()=>onQuick&&onQuick(org,"LIGACAO")} style={{width:44,height:44,borderRadius:50,background:S.pri+"20",border:`1.5px solid ${S.pri}55`,color:S.pl,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><Phone size={26} strokeWidth={2.2}/></button>
        </div>}
        <div style={{display:"flex",gap:4,justifyContent:"flex-end"}}>
          {plocs&&plocs[org.id]&&<button onClick={()=>{const loc=plocs[org.id];const url=`https://www.google.com/maps/dir/?api=1&destination=${loc.lat},${loc.lng}&travelmode=driving`;window.open(url,"_blank","noopener");}} title="Navegar" style={{width:38,height:38,borderRadius:50,background:S.acc+"15",border:`1.5px solid ${S.acc}44`,color:S.acc,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><Navigation size={22} strokeWidth={2.2}/></button>}
          <button onClick={()=>onInfo&&onInfo(org)} style={{width:38,height:38,borderRadius:50,background:S.pl+"10",border:`1.5px solid ${S.brd}`,color:S.pl,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><Info size={22} strokeWidth={2.2}/></button>
          <button onClick={()=>onEdit&&onEdit(org)} style={{width:38,height:38,borderRadius:50,background:S.gold+"10",border:`1.5px solid ${S.brd}`,color:S.gold,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><Pencil size={22} strokeWidth={2.2}/></button>
          <button onClick={()=>onPerson&&onPerson(org)} style={{width:38,height:38,borderRadius:50,background:S.ts+"10",border:`1.5px solid ${S.brd}`,color:S.ts,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><UserPlus size={22} strokeWidth={2.2}/></button>
        </div>
      </div>
    </div>
