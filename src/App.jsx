...
function EditModal({org,token,onSave,onClose}){
  const[name,setName]=useState(org.name||"");const[legal,setLegal]=useState("");const[catId,setCatId]=useState("");const[sectorId,setSectorId]=useState("");const[grupo,setGrupo]=useState(org.grupo?.replace("Grupo: ","")||"");const[ownerId,setOwnerId]=useState("");
  ...
  const save=async()=>{setLo(true);setMsg("");try{const body={};if(name.trim())body.name=name.trim();if(legal.trim())body.legalName=legal.trim();if(catId)body.category=parseInt(catId);if(sectorId)body.sector=parseInt(sectorId);if(ownerId)body.ownerUser=parseInt(ownerId);body.description=grupo.trim()?`Grupo: ${grupo.trim()}`:"";
    await agF(`/organizations/${org.id}`,token,{method:"PUT",body:JSON.stringify(body)});
    const updated={...org,name:name||org.name,grupo:grupo.trim()?`Grupo: ${grupo.trim()}`:""};if(catId)updated.cat=CAT_IDS.find(c=>c.id===parseInt(catId))?.n||org.cat;if(sectorId)updated.sector=SECTORS.find(s=>s.id===parseInt(sectorId))?.n||org.sector;if(ownerId)updated.owner=USERS.find(u=>u.id===parseInt(ownerId))?.n||org.owner;
    onSave(updated);
  }catch(e){setMsg("Erro: "+e.message);}setLo(false);};
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:50,padding:16}}><div style={{background:S.card,borderRadius:16,padding:"1.25rem",width:"100%",maxWidth:420,maxHeight:"90vh",overflowY:"auto"}}>
