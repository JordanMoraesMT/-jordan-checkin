...
export default function App(){
  ...
  const doSync=async(t)=>{setSyncing(true);setSyncMsg("Conectando...");try{let pg=1,all=[];while(true){setSyncMsg(`${all.length} clientes...`);const d=await agF(`/organizations?page=${pg}&per_page=100`,t||token);if(!d.data?.length)break;all.push(...d.data.map(strip).filter(o=>!/exclu/i.test(o.cat||"")));/* v16: Excluído/Excluir fora da carga (resgate pelo botão +) */if(d.data.length<100)break;pg++;}setAllOrgs(all);setOrgs(all);setSyncMsg(`${all.length} clientes`);
  ...
  return(<div style={{minHeight:"100vh",paddingBottom:70}}>
    ...
    <div style={{padding:"0 16px"}}>
      ...
      <CrmTab visible={tab==="crm"} token={token} user={user} allOrgs={allOrgs} visits={visits} plocs={plocs} onEdit={o2=>setEditTarget(o2)} onPerson={o2=>setPersonTarget(o2)} rfv={rfvMap} onNovaEmpresa={()=>setSearchAdd(true)}/>
    ...
    <div style={{position:"fixed",bottom:0,left:0,right:0,background:`linear-gradient(180deg, ${S.chrome}, ${S.chromeTop})`,borderTop:`1px solid rgba(255,255,255,.18)`,display:"flex",justifyContent:"center",zIndex:40,boxShadow:"0 -6px 18px rgba(20,32,43,.20)"}}><div style={{display:"flex",width:"100%",padding:"5px 4px"}}>{tabs.map(t=><button key={t.id} onClick={()=>{setTab(t.id);}} style={{flex:1,border:"none",borderRadius:10,margin:"0 2px",background:tab===t.id?S.navActive:"transparent",padding:"6px 2px 5px",fontSize:10,fontWeight:tab===t.id?700:400,color:tab===t.id?"#fff":S.navFg,display:"flex",flexDirection:"column",alignItems:"center",gap:2,cursor:"pointer"}}><t.I size={22} strokeWidth={tab===t.id?2.2:1.6}/>{t.l}</button>)}</div></div>
