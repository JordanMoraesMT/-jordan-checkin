...
function PeopleModal({org,token,onClose}){
  ...
  useEffect(()=>{(async()=>{try{const d=await agF(`/organizations/${org.id}/people?per_page=50`,token);setPeople(d.data||[]);}catch{}setLo(false);})();},[]);
  const add=async()=>{if(!n.trim())return;setSaving(true);setMsg("");try{await agF("/people",token,{method:"POST",body:JSON.stringify({name:n,organization:org.id,role:cargo||undefined,contact:{...(e?{email:e}:{}),...(p?{mobile:p}:{}),...(w?{whatsapp:w}:{})}})});const d=await agF(`/organizations/${org.id}/people?per_page=50`,token);setPeople(d.data||[]);setAddMode(false);setN("");setCargo("");setE("");setP("");setW("");setMsg("Adicionado!");}catch(x){setMsg("Erro: "+x.message);}setSaving(false);};
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:50,padding:16}}><div style={{background:S.card,borderRadius:16,padding:"1.25rem",width:"100%",maxWidth:420,maxHeight:"90vh",overflowY:"auto"}}>
    ...
    {addMode&&<div style={{background:S.cl,borderRadius:8,padding:10,marginTop:6}}>
      ...
      <LB t="CARGO"><input value={cargo} onChange={x=>setCargo(x.target.value)} style={{width:"100%"}}/></LB>
...
function RelatorioTab({visits,dayBases,user,token,plocs,onEditBase}){
  ...
  const loadRemote=async()=>{setRLo(true);try{
    ...
    const tasks=(d.data||[]).filter(t=>t.user?.id===otherId&&(t.type==="Visita"||t.type==="VISITA"))
...
function ConfigTab({user,orgs,visits,plocs,dayBases,today,syncStatus,syncing,syncMsg,onSync,onLoadHistory,onSyncPull,onShareGPS,onShowDB,onShowEnd,onDeleteGPS,onSaveGPS,onClearVisits,onClearAllGPS,onLogout}){
  ...
  return(<div>
    ...
    <div style={{background:S.card,border:`1px solid ${S.brd}`,borderRadius:12,padding:"1rem",marginBottom:12}}>
      ...
      <p style={{fontSize:10,color:S.td,margin:"2px 0 0"}}>User ID: {user?.id} | Polling: 15s | TZ: Cuiabá | v8.8</p>
    </div>
