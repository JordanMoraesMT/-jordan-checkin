...
function PdvsTab({visible,orgs,allOrgs,setOrgs,visits,plocs,active,ldId,geoErr,user,token,syncing,syncMsg,onSync,onCheckin,onCheckout,onEdit,onPerson,onQuick,focusReq,rfv,excl}){
  ...
  return(<div style={{display:visible?"block":"none"}}>
        ...
        <div style={{display:"flex",gap:6,marginBottom:11,overflowX:"auto",paddingBottom:2,flexWrap:"wrap"}}>
          <button onClick={()=>setCatFilters([])} style={{padding:"6px 13px",fontSize:12,whiteSpace:"nowrap",border:"none",borderRadius:20,fontWeight:600,background:!catFilters.length?"var(--chrome)":S.inp,color:!catFilters.length?"#fff":S.ts,boxShadow:!catFilters.length?"none":`inset 0 0 0 1px ${S.inpBdr}`,cursor:"pointer"}}>Todos</button>
          {CATS.map(c=><button key={c} onClick={()=>toggleCat(c)} style={{padding:"6px 13px",fontSize:12,whiteSpace:"nowrap",border:"none",borderRadius:20,fontWeight:catFilters.includes(c)?600:500,background:catFilters.includes(c)?(CC[c]||S.pri):S.inp,color:catFilters.includes(c)?"#fff":S.ts,boxShadow:catFilters.includes(c)?"none":`inset 0 0 0 1px ${S.inpBdr}`,cursor:"pointer"}}>{c}</button>)}
          <button onClick={()=>{setCatFilters([]);setCityFilter("Todas");setStateFilter("Todos");setSegFilter("Todos");setProdFilter("Todos");setOwnerFilter("Todos");setGrupoFilter("Todos");setVisitMode("all");setSearch("");setSortMode("alpha");setNearMe(null);setFRfv("");setFAbc("");setFSr("");setFInd("");setVc(PG);}} style={{padding:"6px 13px",fontSize:12,border:`1px solid ${S.dng}44`,color:S.dng,borderRadius:20,background:"transparent",whiteSpace:"nowrap",cursor:"pointer"}}>✕ Limpar</button>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:8,marginBottom:11}}>
          <select value={stateFilter} onChange={e=>{setStateFilter(e.target.value);setCityFilter("Todas");setVc(PG);}} style={{fontSize:12.5,padding:"8px 10px",width:"100%"}}>{states.map(s=><option key={s} value={s}>{s==="Todos"?"UF":s}</option>)}</select>
          <select value={cityFilter} onChange={e=>{setCityFilter(e.target.value);setVc(PG);}} style={{fontSize:12.5,padding:"8px 10px",width:"100%"}}>{cities.filter(c=>c==="Todas"||stateFilter==="Todos"||orgs.some(o=>(o.addr?.city_name||o.addr?.city)===c&&o.addr?.state===stateFilter)).map(c=><option key={c} value={c}>{c==="Todas"?"Cidade":c}</option>)}</select>
          <select value={segFilter} onChange={e=>{setSegFilter(e.target.value);setVc(PG);}} style={{fontSize:12.5,padding:"8px 10px",width:"100%"}}>{segments.map(s=><option key={s} value={s}>{s==="Todos"?"Segmento":s}</option>)}</select>
          <select value={prodFilter} onChange={e=>{setProdFilter(e.target.value);setVc(PG);}} style={{fontSize:12.5,padding:"8px 10px",width:"100%"}}>{products.map(p=><option key={p} value={p}>{p==="Todos"?"Produto":p}</option>)}</select>
          <select value={ownerFilter} onChange={e=>{setOwnerFilter(e.target.value);setVc(PG);}} style={{fontSize:12.5,padding:"8px 10px",width:"100%"}}>{owners.map(o=><option key={o} value={o}>{o==="Todos"?"Responsável":o}</option>)}</select>
          <select value={grupoFilter} onChange={e=>{setGrupoFilter(e.target.value);setVc(PG);}} style={{fontSize:12.5,padding:"8px 10px",width:"100%"}}>{grupos.map(g=><option key={g} value={g}>{g==="Todos"?"Grupo":g}</option>)}</select>
          <select value={fRfv} onChange={e=>{setFRfv(e.target.value);setVc(PG);}} style={{fontSize:12.5,padding:"8px 10px",width:"100%"}}><option value="">Classe RFV</option>{["Campeão","Leal","Em Crescimento","Em Risco","Inativo"].map(x=><option key={x} value={x}>{x}</option>)}</select>
          <select value={fAbc} onChange={e=>{setFAbc(e.target.value);setVc(PG);}} style={{fontSize:12.5,padding:"8px 10px",width:"100%"}}><option value="">ABC</option>{["A","B","C"].map(x=><option key={x} value={x}>{x}</option>)}</select>
          <select value={fSr} onChange={e=>{setFSr(e.target.value);setVc(PG);}} style={{fontSize:12.5,padding:"8px 10px",width:"100%"}}><option value="">Recompra</option>{["Em Dia","Momento de Recompra","Atrasado"].map(x=><option key={x} value={x}>{x}</option>)}</select>
          <select value={fInd} onChange={e=>{setFInd(e.target.value);setVc(PG);}} style={{fontSize:12.5,padding:"8px 10px",width:"100%"}}><option value="">Indústria</option>{["TRAMONTINA","PADO","Zagonel","Ruvolo","Santana","Festcolor","Plastilit","Hiper Têxtil"].map(x=><option key={x} value={x}>{x}</option>)}</select>
        </div>
