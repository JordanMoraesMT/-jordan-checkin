...
function EquipeTab({sel,setSel,token,plocs,orgs,dayBases,user}){
  ...
  // Usuário selecionado (dinâmico via catálogo). Default: primeiro que não seja o admin logado, senão o 1º.
  ...
  const load=async()=>{setLo(true);setRouteKm(null);setErr("");try{
    setErr("Buscando...");
    const r=await fetch(`${DASH}/api/crm/atividades?user_id=${uid}&desde=${sel}&limit=2000`,{headers:{"X-Session":token},cache:"no-store"});
    ...
    const lista=deduped.map(t=>({type:t.tipo||"?",org:t.org_nome||"?",orgId:t.org_id,text:t.texto||"",time:t.criado_em,done:t.concluida}));
    setTasks(lista);setErr(`${lista.length} visitas`);
    if(lista.length>=1){const sorted=[...lista].sort((a,b)=>a.time.localeCompare(b.time));let km=0;const startB=getBase(dayBases,sel,uid);const endB=getEnd(dayBases,sel,uid);const fc=getCoord(sorted[0].orgId),lc=getCoord(sorted[sorted.length-1].orgId);
